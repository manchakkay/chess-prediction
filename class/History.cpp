...
void History::format_list()
{
    ...
    for (int l_i = 0; l_i < (int)formatted_list.size(); l_i++)
    {
        for (int f_i = 0; f_i < (int)formatted_list.size(); f_i++)
        {
            if (l_i != f_i)
            {
                if ((formatted_list[l_i].m1 == formatted_list[f_i].m1) && (formatted_list[l_i].m2 == formatted_list[f_i].m2) && (formatted_list[l_i].m3 == formatted_list[f_i].m3))
                {
                    if ((formatted_list[l_i].m1.figure != '0') && (formatted_list[l_i].m2.figure != '0') && (formatted_list[l_i].m3.figure != '0'))
                    {
                        formatted_list[l_i].m3_found++;
                    }
